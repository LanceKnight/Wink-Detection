...
*/
string OPENCV_ROOT = "D:/Ceemple/OpenCV4VS/";
...
bool detectWink(Mat frame, Point location, Mat ROI, CascadeClassifier cascade) {
	...
	vector<Rect> eyes;
	cascade.detectMultiScale(ROI, eyes, 1.035, 7, 0, Size(24, 24)); //detect eyes

	int neyes = (int)eyes.size();
	for (int i = 0; i < neyes; i++) {
		Rect eyes_i = eyes[i];
		drawEllipse(frame, eyes_i + location, 255, 255, 0);//draw light blue for eyes
	}
	return(neyes == 1);
}
...
int detect(Mat frame,
	...
	cascade_face.detectMultiScale(frame_gray, faces,
		1.08, 4, 0 | CV_HAAR_SCALE_IMAGE, Size(30, 30));
	...
	*/
	...
	for (int i = 0; i < nfaces; i++) {
		Rect face = faces[i];
		drawEllipse(frame, face, 255, 0, 255);//draw purple for face
		Mat faceROI = frame_gray(face);
		if (detectWink(frame, Point(face.x, face.y), faceROI, cascade_eyes)) {
			drawEllipse(frame, face, 0, 255, 0);//draw green for wink
			detected++;
		}
	}
	return(detected);
